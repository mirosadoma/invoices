                                        ...
                                        <tbody class="bg-white">
                                            @if($invoice->invoice_activities)
                                                @foreach ($invoice->invoice_activities as $key => $invoice_activity)
                                                    <tr class="bg-primary text-white text-center">
                                                        <th class="p-5 text-base font-semibold border-style">{{$invoice_activity->activity->name}}</th>
                                                        <th class="p-5 text-base font-semibold border-style">{{$invoice_activity->description}}</th>
                                                        <th class="p-5 text-base font-semibold border-style">{{$invoice_activity->price}}</th>
                                                    </tr>
                                                @endforeach
                                            @else
                                                <tr>
                            ...
                            <table>
                                <tbody>
                                    <tr class="cs-table_baseline border_dashed_bottom">
                                        ...
                                        <td class="cs-width_2  cs-primary_color cs-bold cs-primary_color cs-f16" style="text-align: right">{{$invoice->invoice_activities->sum('price') ?? 0}} @lang("R.S")</td>
                                    </tr>
                                    @if ($invoice->is_tax)
                                        <tr class="cs-table_baseline border_dashed_bottom">
                                    ...
                                    <tr class="cs-table_baseline border_solid_top border_solid_bottom">
                                        ...
                                        <td class="cs-width_2  cs-primary_color cs-bold cs-primary_color cs-f16" style="text-align: right">{{ ($invoice->is_tax ? $invoice->invoice_activities->sum('price') + ($invoice->invoice_activities->sum('price')*app_settings()->tax / 100) : $invoice->invoice_activities->sum('price') ?? 0) }} @lang("R.S")</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
